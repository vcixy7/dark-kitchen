...
const { getFirestore, collection, query, where, getDocs, deleteDoc, doc, addDoc } = require('firebase/firestore');
...
// Produtos por restaurante com categorias
...
async function criarProdutos() {
  console.log("🍽️  Criando produtos com categorias...\n");
      ...
      if (userData.tipo === "restaurante" && produtosPorRestaurante[userData.nomeRestaurante]) {
        ...
        for (const produto of produtos) {
          try {
            ...
            const status = produto.disponivel ? "✓" : "✗ INDISPONÍVEL";
            console.log(`   ${status} [${produto.categoria}] ${produto.nome} - R$ ${produto.preco.toFixed(2)}`);
          } catch (error) {
...
async function main() {
  console.log("🚀 Iniciando seed v2 de produtos com categorias\n");

  try {
    await limparProdutosAntigos();
    await criarProdutos();
